...
resource "aws_ecs_task_definition" "ecs_task_definiton" {
    ...
    memory                   = 512
    requires_compatibilities = ["FARGATE"]
    exexution_role_arn       = aws_iam_role.ecs.arn
    task_role_arn            = aws_iam_role.ecs_task.arn
    
    //Dummy Container for initial
    container_definitions = <<CONTAINEERS
[
    {
        "name": "${local.app}",
        "iamge": "medpeer/health_check:latest",
        "portMappings": [
            {
                "hostPort": 8080,
                "containerPort": 8080
...
resource "aws_ecs_service" "ecs_service" {
    ...
    depens_on = [aws_lb_listener_rule.alb_listener_rule]
}
